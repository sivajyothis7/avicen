...
def get_message() -> str:
    ...
    if leave_types:
        message += "<br><br><strong>Leave Types:</strong> "
        leave_abbrs = []
        for lt in leave_types:
            abbr = "".join([word[0].upper() for word in lt.name.split()])
            leave_abbrs.append(f"{lt.name} - {abbr}")
        message += ", ".join(leave_abbrs)
    ...
    for d in attendance_list:
        if d.status == "On Leave":
            leave_data = {
                ...
                "leave_abbr": "".join([word[0].upper() for word in d.leave_type.split()]) if d.leave_type else "L"
            }
    ...
    for employee, leave_days in leave_map.items():
        for assigned_shift, days in leave_days.items():
            ...
            for day_info in days:
                day = day_info["day"]
                leave_data = day_info["data"]
                for shift in attendance_map[employee].keys():
                    attendance_map[employee][shift][day] = leave_data
    ...
    for day in days:
        ...
        for __, attendance_dict in attendance_map.items():
            for __, attendance in attendance_dict.items():
                ...
                if isinstance(attendance_on_day, dict):  
